...
<body>
	...
	<h3>**c:if태그 이용하기**</h3>
	<p>자바에서 if문을 사용한 것과 동일함</p>
	<p>
		c:if<br>
		test : 조건식 -> true, false (EL표현식으로 작성)<br>
	...
	<p>
		<c:if test ="${result }">
			<h1>우하하하 있는 것으로 분기</h1>
		</c:if>
	</p>
	
	<h3>**c:choose태그 이용하기**</h3>
	<p>작성할 땐 java에서 switch문을 사용하는 것과 비슷, 동작은 if~else if~else문과 비슷</p>
	<c:choose>
		<c:when test="${su > 10 }">
			<p>10보다 크다.</p>
		</c:when>
	...
	<p>
		...
		2. 리스트, 배열에 저장된 데이터를 출력(forEach)
	</p>
	<ul>c:forEach태그
		<!-- 기본 for문 사용가능(begin, end, step, var) -->
		<!-- forEach(var, items, varStatus) -->
		<li>begin : 시작번호</li>
		<li>end : 끝번호</li>
		<li>step : 간격</li>
		<li>var : 변경되는 값을 저장하는 변수</li>
		<li>items : 배열, 리스트</li>
		<li>varStatus : 반복문의 정보를 가지고 있는 객체(index, 반복횟수, 시작, 끝 등 정보제공)</li>
	</ul>
	...
	<%
		List<Snack> snack = List.of(
					Snack.builder().type("초콜릿").name("투유").price(1200).weight(80).build(),
