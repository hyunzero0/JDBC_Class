...
<body>
	...
	<h2>내장객체에 중복 키값으로 저장된 데이터 가져오기</h2>
	<p>EL에서 내장객체의 데이터를 가져올 때 작은 범위부터 큰 범위로 탐색함.</p>
	<p>request -> session -> servletContext</p>
	<h3>${snack }</h3>
	<h3>중복된 키값이 있을 때 특정 영역에서 데이터를 찾고싶으면 EL이 제공하는 내장객체를 이용한다</h3>
	<p>
		requestScope : request영역에서만 key검색<br>
		sessionScope : session영역에서만 key검색<br>
		applicationScope : servletContext영역에서만 key검색
	</p>
	...
	<h2>파라미터 값 출력하기</h2>
	<p>별도의 EL객체를 이용해서 출력 -> param객체 이용</p>
