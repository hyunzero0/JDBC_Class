...
public class CsvDataServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		List<Actor> actors = List.of(
					...
					Actor.builder().name("쥴리아로버츠").phone("01055555555").profile("juliaRoberts.jpg").build(),
					Actor.builder().name("멧데이먼").phone("01011111111").profile("mattDamon.jpg").build()
				);
		
		// java코드를 js에서 사용하기 위해 문자열로 바꾸기 -> JSON 나중에 배움
		String csv="";
