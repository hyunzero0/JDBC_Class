...
<script>
	...
	// ajax활용 아이디중복확인 바로 확인하기
	$("#userId_").keyup(e=>{
		if(e.target.value.length >= 4){
			$.ajax({
				url:"<%=request.getContextPath()%>/ajaxDuplicateId.do",
				// member.controller패키지에 서블릿 생성(ajax용 응답인 거 생각하기)
				data:{"userId":$(e.target).val()},
				success:function(data){
					// console.log(data, typeof data); -> String으로 저장
					let msg="", css={};
					if(data==='true'){ // js코드라서 equals 사용x
						msg = "사용가능한 아이디입니다.";
						css = {color:"green"};
					} else {
						msg = "사용 불가능한 아이디입니다.";
					...
					tr.append(td);
					// jsp에 <td>만들고 하는게 좋음(구역)
					// 분기처리 안하면 원래 있던 tr이 사라짐
					if($(e.target).parents("tr").next().find("input").length == 0){
						// jquery는 배열, 아이디 입력하면 msg가 나오기때문에 input태그 찾을 수 없음
						// 그 경우 length == 0;
